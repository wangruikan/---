...
const statefulExclusions = new Set([
  'POST auth/login',
  'POST auth/logout',
  'POST mini/login',
  'PUT roles/menu-layout',
  'DELETE signatures',
  'POST employees/download-all-documents',
  'POST salary-payment-records/export',
]);
const routes = loadActiveApiInventory().usedRoutes.filter((route) => (
  route.method !== 'GET'
  && !route.uri.includes('{')
  && !statefulExclusions.has(`${route.method} ${route.uri}`)
));

let adminHeaders: Record<string, string>;
let miniHeaders: Record<string, string>;

test.describe('Active API invalid-payload mutation contract', () => {
  ...
  for (const route of routes) {
    test(`${route.method} /${route.uri} rejects an empty payload without crashing`, async ({ request }) => {
