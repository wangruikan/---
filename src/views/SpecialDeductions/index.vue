...
const loadProjectEmployees = async () => {
  ...
  try {
    const res = await getProjectEmployees(
      employeeSearchForm.project_id,
      currentAccountSetId.value,
      employeeSearchForm.month,
      'all'
    )
...
onMounted(async () => {
  ...
  if (['employees', 'other'].includes(route.query.tab)) {
